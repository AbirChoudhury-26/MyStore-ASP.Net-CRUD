			...
			try {
				...
				using (SqlConnection connection = new SqlConnection(connectionString))
				{
					...
					String sql = " INSERT INTO clients " +
						"(name,emailId,phone,address) VALUES" + "(@name,@emailId,@phone,@address);"; // @name,@address etc. are the field name that are to be saved with that name only ,which is Given in our Database

					using (SqlCommand command = new SqlCommand(sql, connection))
					{
						command.Parameters.AddWithValue("@name", ClientInfo.name);
						command.Parameters.AddWithValue("@emailId", ClientInfo.email);
